...
function lint_codegen() {
  ...
  echo $(git diff --quiet refs/remotes/origin/HEAD -- services/migration/src/perspex || echo $?)
  if [[ "$(git diff --quiet refs/remotes/origin/HEAD -- services/migration/src/perspex || echo $?)" == "1" ]]; then
    echo "perspex changed"
    genpath="schemas/perspex/pkg/models"
    tool="sqlboiler"
    verify_hashes
  echo $(git diff --quiet refs/remotes/origin/HEAD -- schemas/graphql || echo $?)
  elif [[ "$(git diff --quiet refs/remotes/origin/HEAD -- schemas/graphql || echo $?)" == "1" ]]; then
    echo "graphql changed"
    genpath="schemas/graphql/pkg"
    tool="gqlgen"
    verify_hashes
  echo $(git diff --quiet refs/remotes/origin/HEAD -- schemas/proto/**/*.proto || echo $?)
  elif [[ "$(git diff --quiet refs/remotes/origin/HEAD -- schemas/proto/**/*.proto || echo $?)" == "1" ]]; then
    echo "proto changed"
